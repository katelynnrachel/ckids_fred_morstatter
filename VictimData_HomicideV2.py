...
# Access URL
for i in range(1, 200): # You can change page number here
    ...
    for Name in Nameage:
        if Name.get('href').startswith('/post/'): # URL start with /post/*
            temp = Name.get_text().split(',')
            if(len(temp) == 2):
                secondElement = temp[1]
                if (secondElement.strip().isdigit()):
...
# Output
with open("deathdates-v2.csv", "w", encoding="UTF-8", newline='') as datacsv:
